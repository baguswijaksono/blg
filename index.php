                    ...
                    <div class='max-w-max mx-auto'>
                        <main class='sm:flex'>
                            ...
                            <div class='sm:ml-6'>
                                ...
                                <div class='mt-10 flex space-x-3 sm:border-l sm:border-transparent sm:pl-6'><a href='../' class='inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'>Go back home</a></div>
                            </div>
                        </main>
                    </div>
    ...
    public function header($doc_title)
        ...
        <nav class="flex flex-wrap items-center justify-center px-2 mb-6">
            <div class="container px-4 mx-auto flex flex-wrap items-center justify-center">
                <div class="w-full relative flex justify-center"><button type="button" onclick="toggleNavbar('navbar')" class="rounded-md inline-flex items-center justify-center text-gray-500 md:hidden" aria-expanded="false"><span class="sr-only">Open
                            menu</span><!-- Heroicon name: outline/menu --><svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                ...
                <div class="flex hidden md:block" id="navbar">
                    <div class="flex flex-col md:flex-row mr-auto w-full">
                        ...
                        foreach ($menuItems as $label => $url) {
                            $isActive = ($lastSegment == strtolower($label)) ? 'text-gray-900' : 'text-gray-500 hover:text-gray-900'; ?><a class="text-sm <?= $isActive ?> px-3 py-2 lg:py-1 mx-auto uppercase <?= $isActive == 'text-gray-900' ? 'active' : '' ?>" href="<?= $url ?>"><?= $label ?></a><?php
                                                                                                                                                                                                                                                                            } ?>
                    </div>
                </div>
            </div>
                        ...
                        <div class="w-full pt-4">
                            <div><?php
                                    if ($this->linktreedata !== null) {
                                        foreach ($this->linktreedata as $row) { ?><a href="<?php echo $row['url'] ?>" target="_blank">
                                            <button class="bg-white text-lightBlue-600 font-normal h-10 w-10 items-center justify-center align-center rounded-full outline-none focus:outline-none mr-2" type="button"><i class="fab fa-<?php echo $row['platform_name'] ?>"></i></button></a><?php }
                                                                                                                                                                                                                                                                                        } ?>
                            </div>
                        </div>
                    </div>
            ...
            <?php $this->footer();
                ...
                public function addBlog()
                {
                    if (
                        ...
                        && password_verify($_SESSION['original_password'], $this->getHashedPassword())
                    ) {
                ...
                public function tag($tag)
                {
                    ...
                    <body class="font-sans mx-auto max-w-prose">
                        <div class="pt-12 pb-4"><?php $this->navbar() ?>
                            <div class="px-4 pt-4 prose prose-indigo">
                                ...
                                if ($resultBlogs) { ?> <?php while ($row = $resultBlogs->fetch_assoc()) { ?>
                    ...
                    public function addImage()
                    {
                        if ($_SERVER["REQUEST_METHOD"] == "POST") {
                            ...
                            if (isset($_SESSION['original_password']) && password_verify($_SESSION['original_password'], $this->getHashedPassword())) {
                        ?>
                        ...
                        <body>
                            <div class="container">
                                <form action="<?php echo $form_action; ?>" method="post">
                                    ...
                                    <br>
                                    <input class="form-control" type="text" id="<?php echo $typepost; ?>" name="<?php echo $typepost; ?>" value="<?php echo ($row) ? $row[$typepost] : ''; ?>">
                                    <br>
                                    <label class="form-label" for="docname">Docname:</label>
                                    <br>
                                    <input class="form-control" type="text" id="docname" name="docname" value="<?php echo ($row) ? $row['docname'] : ''; ?>">
                                    <br>
                                    <label class="form-label" for="title">Title:</label>
                                    <br>
                                    <input class="form-control" type="text" id="title" name="title" value="<?php echo ($row) ? $row['title'] : ''; ?>">
                                    <br>
                                    <label class="form-label" for="hypertext">Hypertext:</label>
                                    <br>
                                    <textarea id="hypertext" name="hypertext" rows="4" cols="50"><?php echo ($row) ? $row['hypertext'] : ''; ?></textarea>
                                    <br>
                                    <label class="form-label" for="shortdesc">Short Description:</label>
                                    <br>
                                    <textarea class="form-control" id="shortdesc" name="shortdesc" rows="4" cols="50"><?php echo ($row) ? $row['shortdesc'] : ''; ?></textarea>
                                    <br>
                                    <?php if ($operation == 'edit') : ?> <?php if ($type == 'blog') : ?>
                                            <input name="blog_id" type="hidden" value="<?php echo $row['id'] ?>"><?php endif; ?>
                                        <?php if ($type == 'doc') : ?>
                                            <input name="doc_id" type="hidden" value="<?php echo $row['id'] ?>"><?php endif; ?><?php endif; ?>
                    ...
                    public function editTagPage()
                    {
                        if (password_verify($_SESSION['original_password'], $this->getHashedPassword())) {
                            ...
                            $result = mysqli_stmt_get_result($stmt);
                            while ($row = mysqli_fetch_assoc($result)) { ?>
                                <form action="/editTagHandle" method="POST"><label for="tag_name">tag_name:</label><br><input type="text" id="tag_name" name="tag_name" value="<?php echo $row['tag_name'] ?>"><br><br><input name="tag_id" type="hidden" value="<?php echo $row['id'] ?>"><?php } ?><input type="submit" value="Go"></form>
                            <?php
                        } else {
                            $this->returnToHome();
                        }
                    ...
                    public function manage()
                    {
                        if (isset($_SESSION['original_password'])) { ?>
                            ...
                            if (password_verify($password, $this->getHashedPassword())) {
                                ...
                                <form action="" method="post"><input type="password" name="password" id="password" placeholder="Password"><button type="submit">Unlock</button></form>
                                <?php }
                        }
                        ...
                        public function deleteBlog()
                        {
                            if (password_verify($_SESSION['original_password'], $this->getHashedPassword())) {
                                if (isset($_POST['delete'])) {
                                    ...
                                    $delete_view_query = "DELETE FROM views WHERE content_id = $blog_id";
                                    $delete_view_result = $this->conn->query($delete_view_query);
                                    if ($delete_tags_result && $delete_blog_result && $delete_view_result) {
                                        $this->conn->commit();
                                        $this->returnToHome();
                                        return;
                                    } else {
